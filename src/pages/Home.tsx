...
export function Home() {
  const { isReady, operationLoading, create, getAll, clearStore } =
    useDatabase();
  ...
  const populateSampleData = async () => {
    ...
    try {
      ...
      if (
        ...
        setPopulateStatus(
          "Sample data already exists. Clear data first if you want to repopulate.",
        );
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-lg text-gray-600 mb-8">
        A React application showcasing IndexedDB with custom hooks and React
        Router v7
      </p>
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-gray-600">
            Manage user records with IndexedDB. Create, read, update, and delete
            user data with reactive React hooks.
          </p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-xl font-semibold mb-4">Skills Store</h2>
          <p className="text-gray-600">
            Track skills and competencies. Demonstrates how to work with
            multiple object stores in the same database.
          </p>
        </div>
      </div>
    </div>
